...
export async function POST(req) {
    ...
    try {
        ...
        await pool.query(
            'INSERT INTO order_history (user_id, symbol, quantity, type, date_of_order) VALUES ($1, $2, $3, $4, $5)',
            [curr.rows[0].user_id, symbol.toUpperCase(), quantity, order_type.toLowerCase(), new Date()]
        );

        // Fetch updated holdings for the user
        const updatedHoldings = await pool.query(
            'SELECT symbol, name, quantity, price_change, value FROM holdings WHERE user_id = $1',
            [curr.rows[0].user_id]
        );
